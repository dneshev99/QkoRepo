/*package com.elsys.refpro.refprowatch.test;
...
public class MainActivity extends WearableActivity implements View.OnClickListener {
    ...
    Team homeTeam = new Team(true, "FC Barcelona", "BAR", homePlayer, homeSubstitutes);
    Team awayTeam = new Team(false, "Real Madrid CF", "RMA", awayPlayer, awaySubstitutes);
    ...
    boolean isStarted = false, isHome = true, vibrator = false;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Player defaultPlayer = new Player(1, "Alexander Vervoskiy");
        for (int count =0; count < 11; count++) {
            homePlayer.add(defaultPlayer);
            awayPlayer.add(defaultPlayer);
        }
        for (int count = 0; count < 7; count++) {
            homeSubstitutes.add(defaultPlayer);
            awaySubstitutes.add(defaultPlayer);
        }
        match.getHome().setPlayers(homePlayer);
        match.getHome().setSubstitutes(awaySubstitutes);
        match.getAway().setPlayers(awayPlayer);
        match.getAway().setPlayers(awaySubstitutes);
        ...
        for(int counter = 0; counter < match.getSubstitutesNumber(); counter++) {
            ...
            newSub.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    events = newEvent.addEvent(timers.getMainTimerMinutes() + ":" + timers.getMainTimerSeconds(), getResources().getString(R.string.substitutionEvent), team, playerToSub + "/" + playerName, false);
            ...
            buttonManager.getSubsButtons().add(newSub);  // ADD current substitute to List of Buttons
                        ...
                        alert.setMessage("GOAL").setCancelable(true)
                                .setPositiveButton("GOAL", new DialogInterface.OnClickListener() {
                                    ...
                                    public void onClick(DialogInterface dialog, int which) {
                                        ...
                                        events = newEvent.addEvent(timers.getMainTimerMinutes() + ":" + timers.getMainTimerSeconds() + " / " + match.getHome().getGoals() + ":" + match.getAway().getGoals(), getResources().getString(R.string.goalEvent), team, playerName, false);
                                    }
                                })
                                .setNegativeButton("OWN GOAL", new DialogInterface.OnClickListener() {
                                    ...
                                    public void onClick(DialogInterface dialog, int which) {
                                        ...
                                        ownGoal = true;
                                        changeResult(ownGoal);
                                        events = newEvent.addEvent(timers.getMainTimerMinutes() + ":" + timers.getMainTimerSeconds() + " / " + match.getHome().getGoals() + ":" + match.getAway().getGoals(), getResources().getString(R.string.ownGoalEvent), team, playerName, false);
                                    }
                                });
